...
Citizen.CreateThread(function()
	while DBFW == nil do TriggerEvent('DBFW:GetObject', function(obj) DBFW = obj end) Wait(0) end
   DBFW.Functions.TriggerCallback('nuclear:getlocation', function(servercoords)
        coords = servercoords
	end)
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if DBFW ~= nil then
			local requiredItems = {
				[1] = {name = DBFW.Shared.Items["bluechip"]["name"], image = DBFW.Shared.Items["bluechip"]["image"]},
			}
		...
		if not inUse then
			if dist <= 1.3 then										 	
				...
				if IsControlJustPressed(0, 51) then
					DBFW.Functions.TriggerCallback('DBFW:HasItem', function(result)
						if result then
							decipherAnim()
							main()
					else
						DBFW.Functions.Notify("You need blue encryption chip.", "error")
						inUse = false
						end
				end, 'bluechip')
				Citizen.Wait(10700)									
...
function main()
	...
	if Config.useNotification then
		DBFW.Functions.Notify("Go to the highlighted area to search for the crate.", "error", 2500)
	end
	local player = GetPlayerPed(-1)
	local playerpos
	enroute = true
	...
	Citizen.CreateThread(function()
		while enroute == true do
			...
			if disttocoord < Config.distance then
				if Config.useNotification then
					DBFW.Functions.Notify("Kill all the enemies to steal.", "error", 2500)
				end
		...
		while inUse do
			...
			if IsEntityDead(player) then
				...
				if disttocoord > Config.maxDistance and not enroute then
					DBFW.Functions.Notify("You went far away from the location.", "error", 2500)
					maxDist()
					return
				end
			end
...
function success(x,y,z,h)
	...
	local player = GetPlayerPed(-1)
	if Config.useNotification then
		DBFW.Functions.Notify("Search for the nuclear files.", "error", 2500)
	end
		...
		while not crate do 
			...
			if disttocoord <= 3 then
				...
				if IsControlJustPressed(1, 51) then
					...
					DeleteEntity(box2)
					if Config.useNotification then
						DBFW.Functions.Notify("You received nuclear files. Now get to safety.", "error", 2500)
					end
...
function decipherAnim()
	...
	DBFW.Functions.Progressbar("hack", "Deciphering Location", 10500, false, true, {
...
function spawnPed(x,y,z)
	...
	DBFW.Functions.TriggerCallback("nuclear:getCops", function(getCops)
